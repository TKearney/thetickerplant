...
//console.log(yVals)
  ...
  zScale.domain([d3.min(zVals), d3.max(zVals)]).range([height-(margin.top+margin.bottom), 0])
  xAxisCall.scale(xScale)
  yAxisCall.scale(yScale)    
  zAxisCall.scale(zScale)    
}
function updateAxis(){
  var t = d3.transition()
   .duration(500)
...
function initAxis()
{
  ...
  zScale.domain([0, 100]).range([height-(margin.top+margin.bottom), 0])
  xAxisCall.scale(xScale)
  yAxisCall.scale(yScale)
  zAxisCall.scale(zScale)
...
var line2 = d3.line()
    .x(function(d) { return xScale(d.time); }) // set the x values for the line generator
    .y(function(d) { return zScale(d.humidity); }) // set the y values for the line generator 
    .curve(d3.curveMonotoneX) // apply smoothing to the line
